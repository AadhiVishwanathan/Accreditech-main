...
-- Create new function with fixed column ambiguity and notification sending
...
BEGIN
  ...
  -- Upsert evaluation using constraint name to avoid ambiguity
  ...
  -- Determine notification details based on evaluation
  IF p_is_approved THEN
    ...
    notif_message := 'Congratulations! Your application has been approved by the expert evaluation committee.';
    notif_type := 'success';
  ELSIF p_requires_rescheduling THEN
    ...
    notif_message := 'The expert visit has been rescheduled' || 
                     CASE WHEN p_new_proposed_date IS NOT NULL 
                          THEN ' to ' || to_char(p_new_proposed_date, 'DD Mon YYYY at HH24:MI')
                          ELSE '' END ||
  ...
  ELSE
    ...
    notif_title := 'Evaluation Complete - Action Required';
    notif_message := 'The expert evaluation has been completed. Please review the feedback and make necessary modifications.';
    notif_type := 'info';
  END IF;
  ...
  -- Find institute user to send notification
  ...
  -- Send notification to institute dashboard
  ...
  -- Return the evaluation record with prefixed column names
  RETURN QUERY SELECT 
