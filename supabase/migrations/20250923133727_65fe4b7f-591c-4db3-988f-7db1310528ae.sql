-- Add trigger to automatically calculate AI scores when application data changes
...
-- Create trigger for automatic AI score calculation
DROP TRIGGER IF EXISTS trigger_calculate_ai_score ON applications;
CREATE TRIGGER trigger_calculate_ai_score
  BEFORE INSERT OR UPDATE ON applications
  FOR EACH ROW
  EXECUTE FUNCTION calculate_ai_score_for_application();

-- Update existing applications with AI scores
UPDATE applications 
SET ai_score = calculate_ai_score_for_application_batch.score
FROM (
  SELECT 
    id,
    CASE 
      WHEN faculty_data IS NOT NULL AND faculty_data != '{}' 
           AND infrastructure_data IS NOT NULL AND infrastructure_data != '{}'
           AND documents IS NOT NULL AND documents != '{}' THEN
        ROUND(
          (
            -- Faculty score (30% weight)
            (CASE 
              WHEN (faculty_data->>'total_faculty')::INTEGER >= 20 THEN 85
              WHEN (faculty_data->>'total_faculty')::INTEGER >= 15 THEN 75
              WHEN (faculty_data->>'total_faculty')::INTEGER >= 10 THEN 65
              WHEN (faculty_data->>'total_faculty')::INTEGER >= 5 THEN 55
              ELSE 45
            END * 0.3) +
            -- Infrastructure score (40% weight)
            (CASE 
              WHEN (infrastructure_data->>'total_area')::NUMERIC >= 10000 THEN 90
              WHEN (infrastructure_data->>'total_area')::NUMERIC >= 5000 THEN 80
              WHEN (infrastructure_data->>'total_area')::NUMERIC >= 2000 THEN 70
              WHEN (infrastructure_data->>'total_area')::NUMERIC >= 1000 THEN 60
              ELSE 50
            END * 0.4) +
            -- Document score (30% weight)
            (LEAST(95, 50 + (jsonb_array_length(documents) * 5)) * 0.3)
          ), 1
        )
      ELSE NULL
    END as score
  FROM applications
) as calculate_ai_score_for_application_batch
WHERE applications.id = calculate_ai_score_for_application_batch.id;