-- Drop existing function and recreate with proper return type and notification
DROP FUNCTION public.submit_evc_evaluation(uuid,uuid,timestamp with time zone,text,text,text[],text[],integer,integer,integer,boolean,boolean,text,timestamp with time zone);
...
BEGIN
  ...
  IF p_is_approved THEN
    ...
    notif_message := 'Your application has been approved by the expert committee.';
    notif_type := 'success';
  ELSIF p_requires_rescheduling THEN
    ...
    notif_message := 'Expert visit has been rescheduled' ||
                     CASE WHEN p_new_proposed_date IS NOT NULL THEN ' to ' || to_char(p_new_proposed_date, 'DD Month YYYY') ELSE '' END ||
                     CASE WHEN p_reschedule_reason IS NOT NULL THEN '. Reason: ' || p_reschedule_reason ELSE '' END;
    notif_type := 'warning';
  ELSE
    ...
    notif_message := 'Expert evaluation has been completed. Please review the feedback and make necessary improvements.';
    notif_type := 'info';
  END IF;
