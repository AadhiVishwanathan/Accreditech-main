-- Replace submit_evc_evaluation to fix ambiguous column and send notification
...
BEGIN
  ...
  IF p_is_approved THEN
    ...
    notif_message := 'Your application ' || evaluation_record.application_id || ' has been approved.';
    notif_type := 'success';
  ELSIF p_requires_rescheduling THEN
    ...
    notif_message := 'Expert visit has been rescheduled to ' || to_char(COALESCE(p_new_proposed_date, p_visit_date), 'YYYY-MM-DD"T"HH24:MI:SSOF') ||
                     CASE WHEN p_reschedule_reason IS NOT NULL THEN ' - ' || p_reschedule_reason ELSE '' END;
    notif_type := 'warning';
  ELSE
    ...
    notif_title := 'Evaluation Submitted';
    notif_message := 'Evaluation submitted with remarks: ' || COALESCE(p_remarks, 'No remarks');
    notif_type := 'info';
  END IF;
...
-- Re-grant execute in case function replaced
